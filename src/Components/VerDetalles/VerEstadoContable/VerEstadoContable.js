...
const MyDocument = ({ estadoContable }) => (
  <Document>
    <Page size="A4" style={{ padding: 30 }}>
      ...
      <View>
        <Text style={{ fontSize: 16, marginBottom: 10 }}>Egresos:</Text>
        {estadoContable.egresos?.length > 0 ? (
          estadoContable.egresos.map((egreso, index) => (
            <Text key={index} style={{ fontSize: 12, marginLeft: 20 }}>
              {egreso.descripcion}: {egreso.monto}
            </Text>
          ))
        ) : (

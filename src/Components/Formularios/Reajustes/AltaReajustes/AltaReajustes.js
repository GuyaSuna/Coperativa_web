...
const AltaReajuste = ({ ur, setIdentificadorComponente }) => {
  ...
  const [
    cuotaMensualDosHabitacionesEnPesos,
    setCuotaMensualDosHabitacionesEnPesos,
  ] = useState("");
  const [
    cuotaMensualTresHabitacionesEnPesos,
  ...
  const fetchReajuste = async () => {
    ...
    if (response) {
      setCuotaMensualDosHabitacionesEnPesos(
        response.cuotaMensualDosHabitacionesEnPesos.toFixed(2)
      );
      setCuotaMensualTresHabitacionesEnPesos(
        response.cuotaMensualTresHabitacionesEnPesos.toFixed(2)
      );
    } else {
      setCuotaMensualDosHabitacionesEnPesos("");
      setCuotaMensualTresHabitacionesEnPesos("");
  ...
  const validarFormulario = () => {
    ...
    if (!cuotaMensualDosHabitacionesEnPesos) {
      errores.cuotaMensualDosHabitacionesEnPesos =
        "La cuota de 2 habitaciones es obligatoria";
    } else if (
      isNaN(cuotaMensualDosHabitacionesEnPesos) ||
      cuotaMensualDosHabitacionesEnPesos <= 0
    ) {
      errores.cuotaMensualDosHabitacionesEnPesos =
    ...
    if (!cuotaMensualTresHabitacionesEnPesos) {
      errores.cuotaMensualTresHabitacionesEnPesos =
        "La cuota de 3 habitaciones es obligatoria";
    } else if (
      isNaN(cuotaMensualTresHabitacionesEnPesos) ||
      cuotaMensualTresHabitacionesEnPesos <= 0
    ) {
      errores.cuotaMensualTresHabitacionesEnPesos =
        "Debe ser un número mayor que 0";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      cuotaMensualDosHabitacionesEnPesos: parseFloat(
        cuotaMensualDosHabitacionesEnPesos
      ),
      cuotaMensualTresHabitacionesEnPesos: parseFloat(
        cuotaMensualTresHabitacionesEnPesos
      ),
    };
    ...
    <div className="max-h-screen items-center justify-center bg-white dark:bg-gray-800 text-black dark:text-white">
      ...
      >
        ...
        <div className="mb-4">
          <label
            className="block text-sm font-medium mb-2"
            htmlFor="cuotaMensualDosHabitacionesEnPesos"
          >
          ...
          <input
            ...
            onChange={(e) =>
              setCuotaMensualDosHabitacionesEnPesos(e.target.value)
            }
            className="w-full p-2 border border-gray-300 rounded-md dark:border-gray-700 dark:bg-gray-800 dark:text-white"
          />
          {errores.cuotaMensualDosHabitacionesEnPesos && (
        ...
        <div className="mb-4">
          <label
            className="block text-sm font-medium mb-2"
            htmlFor="cuotaMensualTresHabitacionesEnPesos"
          >
          ...
          <input
            ...
            onChange={(e) =>
              setCuotaMensualTresHabitacionesEnPesos(e.target.value)
            }
            className="w-full p-2 border border-gray-300 rounded-md dark:border-gray-700 dark:bg-gray-800 dark:text-white"
          />
          {errores.cuotaMensualTresHabitacionesEnPesos && (

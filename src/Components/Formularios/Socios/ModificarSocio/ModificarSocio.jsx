...
const ModificarSocio = ({ socio, setIdentificadorComponente }) => {
  ...
  const fetchViviendas = async () => {
    ...
    const viviendaSocioFilter = response.find(
      (vivienda) => vivienda.socio?.cedulaSocio === socio.cedulaSocio
    );
        ...
        <div className="grid md:grid-cols-2 md:gap-6">
          ...
          <div className="grid md:grid-cols-2 md:gap-6">
            <label className="block text-sm font-medium mb-2">
              ...
              {errores.idVivienda && (
                <span className="error">{errores.idVivienda}</span>
              )}
            </label>
          </div>
        </div>
